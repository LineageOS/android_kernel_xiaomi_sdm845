...
static ssize_t nvt_panel_dt2w_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
...
static ssize_t nvt_panel_dt2w_store(struct device *dev,
...
static DEVICE_ATTR(wake_gesture, S_IWUSR | S_IRUSR, nvt_panel_dt2w_show, nvt_panel_dt2w_store);
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("%s: Failed to symlink input device\n",
				__func__);
	}
